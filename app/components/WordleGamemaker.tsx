...
const Intro: FC<{onClick: MouseEventHandler<HTMLButtonElement>}> = ({onClick}) => <div className="text-justify">
  <p className="mb-3 font-bold">Welcome to the Wordle gamemaker wizard!</p>
  <p className="mb-3">You will now be taken through the necessary steps to set up a game of Nillion Wordle:</p>
  ...
  <p className="mb-3">This will require you to spend some testnet NIL for the transactions.</p>
  <p className="mb-3">After restarting Wordle with the correct environment variables, it will be ready to play.</p>
  <button className="flex items-center justify-center px-3 py-1 border rounded text-black text-center my-1 bg-white hover:bg-gray-100 mx-auto" onClick={onClick}>Got it</button>
</div>
  ...
  return (
    <div className="flex items-center justify-center mt-3">
      ...
      >{
        ...
        ? "Stored!"
        : nilStoreProgram.isLoading
          ? <div className="flex flex-row"><div className="w-5 h-5 border-t-2 border-b-2 border-gray-900 rounded-full animate-spin display-inline mr-3"></div>Waiting for transaction...</div>
          : "Store program"
      }</button>
    </div>
  )
...
const Final = () => {
  ...
  return <div>
    <p className="text-left mb-3">Now pass <b>only</b> these environment variables to this app in <code>.env</code> or otherwise and restart:</p>
    <pre className="text-left rounded border py-2 px-3 mb-5">{envvarsJoined}</pre>
    <p className="text-left mb-3"><i>Note:</i> especially make sure that the <code>NEXT_PUBLIC_WORDLE_GAMEMAKER_USER_SEED</code> variable is commented out or otherwise not set.</p>
    <button
...
export default function WordleGamemakerWizard() {
  ...
  if (step === 1 && wordle.programId)
    setStep(2)

  if (step === 2 && wordle.gmStoreId)
    setStep(3)

  return <div className="border border-gray-400 rounded-lg p-4 w-full text-center">{steps.at(step)}</div>
}
